...
void fraction::showFraction(){
	lowterms();
	cout << setiosflags(ios::right) << setw(3) << num << "/"; 
	cout << setiosflags(ios::left) << setw(3) << den;
}
...
int main(){
	...
	for (int i = 1; i < N; i++){
		fraction f(i, N);
		f.showFraction(); cout << setw(width) << "";  
	}
	cout << endl;

	// seperation line
	cout << setw((width+7)*N) << setfill('-') << "" << endl;
	cout << setfill(' ');

	// print the table
	for (int i = 1; i < N; i++){
		fraction f1(i, N);
		f1.showFraction(); cout << setw(width) << ""; 

		for (int j = 1; j < N; j++){
			fraction f2(j, N);
			fraction f3;
			f3.fmul(f1, f2);
			f3.showFraction();cout << setw(width) << ""; 
		}
		cout << endl;
	}
